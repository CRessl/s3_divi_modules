

<?php if($this->e($item['first_post'])): ?>
    
    <div class="uk-position-relative">
        <a href="<?= $this->e($item['link']); ?>" class="uk-position-cover"></a>
        <div class="s3dm_post_list_headline_wrapper <?= $this->e($settings['title_size']); ?>">    
          <h3 class="s3dm_post_list_headline"><?= $this->e($item['title']); ?></h3>
        </div>
        <div class="tags-container uk-margin-bottom"> 
        <?php if($item['tags'] && $settings['show_tags']):?>
          <?= $item['tags'] ?>
        <?php endif; ?>
        </div>
        <?= $item['image'] ?>

    </div>

<?php else: ?>
    
    <div class="uk-position-relative uk-margin-bottom s3dm_post_list_right_item">
        
        <a href="<?= $this->e($item['link']); ?>" class="uk-position-cover"></a>    
        <div class="s3dm_post_list_headline_wrapper <?= $this->e($settings['title_size']); ?>">    
          <h3 class="s3dm_post_list_headline"><?= $this->e($item['title']); ?></h3>
        </div>
        <div class="tags-container">
        <?php if($item['tags'] && $settings['show_tags']):?>
          <?= $item['tags'] ?>
        <?php endif; ?>
        </div>

    </div>

<?php endif; ?>

